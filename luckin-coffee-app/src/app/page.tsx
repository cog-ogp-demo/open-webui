...
export default function Home() {
	...
	function handleUpdateQuantity(productId: number, amount: number) {
		setCart((prev) =>
			prev.map((item) => (item.productId === productId ? { ...item, amount } : item))
		);
	}
	...
	return (
		<div className="min-h-screen bg-gray-50">
			...
			<main className="max-w-2xl mx-auto px-4 py-6">
				{step === 'shop' && (
					<div>
						<h2 className="text-lg font-semibold text-gray-900 mb-4">Find a Store</h2>
				...
				{step === 'menu' && shop && (
					<div className="space-y-6">
						<div>
							...
							<ProductSearch token={token} shop={shop} cart={cart} onAddToCart={handleAddToCart} />
						</div>
				...
				{step === 'success' && order && (
					<OrderSuccess token={token} order={order} onNewOrder={handleNewOrder} />
				)}
			</main>
		</div>
	);
