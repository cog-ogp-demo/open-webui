export async function mcpCall(token: string, tool: string, args: Record<string, unknown> = {}) {
...
export async function searchProducts(token: string, deptId: number, query: string) {
	return mcpCall(token, 'searchProductForMcp', { deptId, query });
}

export async function getProductDetail(token: string, deptId: number, productId: number) {
