...
export default function Cart({ items, onUpdateQuantity, onRemove, onCheckout }: CartProps) {
	const total = items.reduce((sum, item) => sum + item.estimatePrice * item.amount, 0);
			...
			{items.map((item) => (
				<div key={item.productId} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
					...
					<div className="flex-1 min-w-0">
						...
						<p className="text-sm text-blue-600">¥{item.estimatePrice}</p>
					</div>
					<div className="flex items-center gap-2">
						...
						<span className="text-sm font-medium w-4 text-center text-gray-900">{item.amount}</span>
						<button
							onClick={() => onUpdateQuantity(item.productId, item.amount + 1)}
